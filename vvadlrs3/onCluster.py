...
if __name__ == "__main__":
    ...
    parser.add_argument("task", help='what should be done', type=str,
                        choices=['timeSteps', 'imageSize', 'bestLipEndToEnd',
                                 'bestFaceEndToEnd', 'FeatureTimeSteps',
                                 'bestFeatures', 'bestFaceEndToEndFromCheckpoint'])
    parser.add_argument("-m", "--base_model", help="name of the base model", type=str)
    parser.add_argument("-s", "--step_size",
                        help="step size for the timeSteps or imageSize", type=int)
    parser.add_argument("-f", "--frames",
                        help="Number of frames to evaluate the image size on", type=int)
    parser.add_argument("-g", "--start",
                        help="start from this number of frames", type=int)
    parser.add_argument("-d", "--dataPath", help="path to the dataset", type=str)
    parser.add_argument("-n", "--modelName",
                        help="path to the location where the best model should be "
                             "saved", type=str)
    ...
    if args.task == 'timeSteps':
        ...
        for timeStep in timeSteps:
            ...
            validation_data_parted = (validation_data[0][:, :timeStep],
                                      validation_data[1])

            input_shape = train_x_parted[0].shape
            print(input_shape)
            epochs = 200
            model, modelName = Models.build_time_distributed(
                args.base_model,
            ...
            history = model.fit(x=train_x_parted, y=train_y_parted,
                                validation_data=validation_data_parted,
                                batch_size=batch_size, epochs=epochs)
            num_samples = 'all' if not train_samples else train_samples
            Models.save_history(history.history,
                                "../trainingHistories/onCluster_{}_{}_{}_{}_{}."
                                "pickle".format(num_samples, modelName,
        ...
        for image_size in image_sizes:
            ...
            model, modelName = Models.build_baseline_model('mobileNet',
                                                           input_shape=input_shape,
            ...
            history = model.fit(x=train_x_resized, y=train_y_resized,
                                validation_data=validation_data_resized,
                                batch_size=batch_size, epochs=epochs)
            num_samples = 'all' if not train_samples else train_samples
            Models.save_history(history.history,
                                "../trainingHistories/IMAGEEVAL{}_{}_{}_{}_{}."
                                "pickle".format(num_samples, modelName,
                                                batch_size, input_shape,
                                                datetime.datetime.now()))
            print("Needed {}s to train a model".format(time.time() - start))

            # training_generator = DataGeneratorRAM((train_x, train_y), imageSize =
            #   (image_size, image_size),
            #   num_steps=num_steps, batch_size=64)
            # validation_generator = DataGeneratorRAM(validation_data, imageSize =
            #   (image_size, image_size),
            #   num_steps=num_steps, batch_size=64)
            ...
            #                     workers=max([multiprocessing.cpu_count() - 5, 10]),
            #                     epochs=epochs)
            # num_samples = 'all' if not train_samples else train_samples
            # Models.saveHistory(history.history, "../trainingHistories/
            #   onClustercheckImageSize{}_{}_{}_{}_{}.pickle".
            #   format(num_samples, modelName, batch_size, input_shape,
            #   datetime.datetime.now()))
        ...
        reduce_lr = callbacks.ReduceLROnPlateau(monitor='val_acc',
                                                min_lr=0.001, cooldown=2)
        earlyStopping = callbacks.EarlyStopping(monitor='val_acc', patience=30,
                                                restore_best_weights=True)

        checkpoint = callbacks.ModelCheckpoint(args.task + ".h5", monitor='val_acc',
                                               save_best_only=True)

        history = model.fit(x=train_x, y=train_y, validation_data=validation_data,
                            batch_size=batch_size, epochs=epochs,
                            callbacks=[reduce_lr, earlyStopping, checkpoint])
        num_samples = 'all' if not train_samples else train_samples
        Models.save_history(history.history,
                            "../trainingHistories/onCluster_{}_{}_{}_{}_{}_{}."
                            "pickle".format(args.task, num_samples,
        ...
        for timeStep in timeSteps:
            ...
            print(input_shape)
            epochs = 200
            model, modelName = Models.build_feature_lstm(input_shape=input_shape)
            ...
            Models.save_history(history.history,
                                "../trainingHistories/"
                                "onClusterFeatureLSTM_{}_{}_{}_{}_{}.pickle".format(
                                    num_samples,
                                    modelName,
                                    batch_size,
                                    input_shape,
        ...
        checkpoint = callbacks.ModelCheckpoint(args.modelName + ".h5",
        ...
        #                                             #num_dense_layers=0,
        #                                             #dense_dims=512,
        #                                             #base_model_weights='../models/
        #                                             mobileNetBaselineWeights.h5'
        #                                             )
        ...
        checkpoint = callbacks.ModelCheckpoint(filepath, monitor='val_acc')
